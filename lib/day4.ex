defmodule Aoc2020.Day4 do
  ...
  def part1 do
    Enum.count(passports(), &valid_passport?/1)
  end

  def part2 do
    Enum.count(passports(), &valid_passport_2?/1)
  end

  def valid_passport?(passport) do
    @required_keys
    |> Enum.all?(&Keyword.has_key?(passport, String.to_atom(&1)))
  end

  def valid_passport_2?(passport) do
    Enum.all?(@validations, fn {key, f} ->
      case Keyword.get(passport, key) do
        nil -> false
        value -> apply(__MODULE__, f, [value])
      end
    end)
  end

  def validate_birth_year(year) do
    between(year, 1920, 2002)
  end

  def validate_issue_year(year) do
    between(year, 2010, 2020)
  end

  def validate_expiration_year(year) do
  ...
  def validate_hair_color(color) do
    Regex.match?(~r/^#[0-9a-f]{6}$/, color)
  end

  def validate_eye_color(color) do
  ...
  defp between(value, min, max) when is_binary(value) do
  ...
  def parse_passport(line) do
    @required_keys
    |> Enum.map(fn key ->
      {:ok, pattern} = Regex.compile("(#{key}):([^\s\n]+)")
      matches = Regex.run(pattern, line)

      case matches do
        nil -> nil
        [_, key, value] -> {String.to_atom(key), value}
      end
    end)
