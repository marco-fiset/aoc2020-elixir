defmodule Aoc2020.Day2 do
  def part1 do
    count_valid_passwords(parsed_input(), &validate_password/2)
  end

  def part2 do
    count_valid_passwords(parsed_input(), &validate_password_2/2)
  end

  def count_valid_passwords(passwords, validate_fn) do
    Enum.count(passwords, &validate_fn.(&1.password, &1.policy))
  end

  def validate_password(password, %{letter: letter, min: min, max: max}) do
    count = String.length(password) - String.length(String.replace(password, letter, ""))
    min <= count && count <= max
  end

  def validate_password_2(password, %{letter: letter, min: pos1, max: pos2}) do
    valid_positions = [pos1, pos2]
  ...
  def parse_line(line) do
    ...
    [_, min, max, letter, password] = Regex.run(pattern, line)
